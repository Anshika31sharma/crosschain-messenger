...
const CONTRACT_ADDRESSES: Record<string, string> = {
  [CHAIN_ID_LOCALHOST]: "0x70997970C51812dc3A010C7d01b50e0d17dc79C8",
  [CHAIN_ID_AMOY]: "0x1CBd3b2770909D4e10f157cABC84C7264073C9Ec",
};
...
export async function sendCrossChainMessage(
  ...
  const dstContract = new Contract(
    CONTRACT_ADDRESSES[dstChain],
    ABI,
    dstProvider
  );
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      logger(
        "⚠️ MessageReceived not emitted in time. Check destination contract or wait longer."
      );
      resolve(tx.hash);
    }, 20000);
  });
